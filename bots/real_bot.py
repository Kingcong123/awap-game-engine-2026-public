...
class BotPlayer:
    ...
    def play_turn(self, controller: RobotController):
        ...
        # Initialize locations once - find nearest tiles to midpoint of both bots
        if self.shop_loc is None:
            pstate = controller.get_bot_state(self.provider_bot_id)
            px, py = pstate['x'], pstate['y']

            # If we have an assembler, use midpoint between bots
            if self.assembler_bot_id:
                astate = controller.get_bot_state(self.assembler_bot_id)
                ax, ay = astate['x'], astate['y']
                cx, cy = (px + ax) // 2, (py + ay) // 2
            else:
                cx, cy = px, py

            # Find nearest of each type to center point
            def nearest(locs):
                if not locs:
                    return None
                return min(locs, key=lambda p: abs(p[0]-cx) + abs(p[1]-cy))
            ...
            if counters:
                ...
                sorted_counters = sorted(counters, key=lambda p: abs(p[0]-cx) + abs(p[1]-cy))
